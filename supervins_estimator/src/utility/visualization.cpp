...
ros::Publisher pub_superpoint_descriptors;
...
{
    pub_superpoint_descriptors = n.advertise<sensor_msgs::Image>("superpoint_descriptors", 1000);
...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        // write result to file(modified to fit evo)
        ofstream foutC(VINS_RESULT_PATH, ios::app);
        foutC.setf(ios::fixed, ios::floatfield);
...
void pubSuperPointDescriptors(Estimator &estimator, std_msgs::Header header)
{
    std::vector<std::pair<double, cv::Mat>> temp_superpoint_descriptors;

    estimator.mSuperPointDescriptors.lock();
    while (!estimator.SuperPointDescriptorsBuf.empty())
    {
        temp_superpoint_descriptors.push_back(estimator.SuperPointDescriptorsBuf.front());
        estimator.SuperPointDescriptorsBuf.pop();
    }
    estimator.mSuperPointDescriptors.unlock();

    for (const auto &pair : temp_superpoint_descriptors)
    {
